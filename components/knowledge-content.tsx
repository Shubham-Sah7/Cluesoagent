  ...
  return (
    <main className="flex-1 overflow-auto">
      <div className="max-w-[1400px] mx-auto px-16 py-10">
        ...
        <section className="mb-12">
          <div className="bg-white border border-[#E8E8E6] rounded-2xl p-10">
            <div className="max-w-3xl mx-auto text-center mb-8">
              <div className="inline-flex items-center gap-2 bg-[#F5F5FF] text-[#8F8CFF] px-4 py-2 rounded-full text-[13px] font-medium mb-4">
            ...
            <div className="relative max-w-2xl mx-auto mb-6">
              ...
              <input
                ...
                className="w-full pl-14 pr-14 py-4 text-[15px] bg-[#F5F5F3] border-2 border-transparent rounded-xl focus:outline-none focus:ring-2 focus:ring-[#8F8CFF] focus:bg-white focus:border-[#8F8CFF] transition-all"
              />
              <button className="absolute right-3 top-1/2 -translate-y-1/2 p-2 bg-[#8F8CFF] text-white rounded-lg hover:bg-[#7B77FF] transition-colors">
        ...
        <section className="mb-12">
          <div className="flex items-center gap-2.5 mb-5">
            <Zap className="w-5 h-5 text-[#8F8CFF]" strokeWidth={1.5} />
        ...
        <section className="mb-12">
          <div className="flex items-center gap-2.5 mb-5">
            <Network className="w-5 h-5 text-[#8F8CFF]" strokeWidth={1.5} />
...
function ExampleQuery({ text }: ExampleQueryProps) {
  return (
    <button className="text-[12px] text-[#71717A] bg-white border border-[#E8E8E6] px-3 py-1.5 rounded-full hover:border-[#8F8CFF] hover:text-[#8F8CFF] transition-all">
      {text}
    </button>
  )
}
...
function OverviewCard({ icon, label, count, growth, active }: OverviewCardProps) {
  return (
    <div className={`bg-white border rounded-lg p-5 transition-all ${
      ...
      <div className={`p-2.5 rounded-lg inline-flex mb-3 ${active ? "bg-[#F5F5FF]" : "bg-[#F5F5F3]"}`}>
  ...
  return (
    <button
      ...
      className={`flex items-center gap-3 p-4 rounded-lg border transition-all text-left ${
        active
          ? "bg-[#F5F5FF] border-[#8F8CFF] shadow-sm"
          : highlight
          ? "bg-[#F5F5FF] border-[#E8E7FF] hover:border-[#8F8CFF]"
          : "bg-white border-[#E8E8E6] hover:border-[#D4D4D2]"
      }`}
    >
      <div className={`p-2 rounded-md ${active || highlight ? "bg-white" : "bg-[#F5F5F3]"}`}>
        <div className={active || highlight ? "text-[#8F8CFF]" : "text-[#52525B]"}>
          {icon}
        </div>
      </div>
      ...
      <div className="flex items-start gap-3 mb-4">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 mb-1">
            <span className="text-[11px] text-[#8F8CFF] font-medium">Generated from</span>
            <div className="text-[#8F8CFF]">{getIcon()}</div>
          </div>
...
function AIRecommendationCard({ title, description, action }: AIRecommendationCardProps) {
  return (
    <div className="bg-white border border-[#E8E8E6] rounded-lg p-5 hover:border-[#8F8CFF] hover:shadow-md hover:-translate-y-0.5 transition-all cursor-pointer group">
      <div className="flex items-start gap-3 mb-4">
        <div className="p-2 bg-[#F5F5FF] rounded-md text-[#8F8CFF]">
...
function TopicCard({ title, count }: TopicCardProps) {
  return (
    <button className="bg-white border border-[#E8E8E6] rounded-lg p-4 hover:border-[#8F8CFF] hover:shadow-sm hover:-translate-y-0.5 transition-all text-left group">
      <div className="font-medium text-[14px] text-[#18181B] mb-1 group-hover:text-[#8F8CFF] transition-colors">
        {title}
      </div>
  ...
  return (
    <div className="group bg-white border border-[#E8E8E6] rounded-lg p-5 hover:border-[#D4D4D2] hover:shadow-md hover:-translate-y-0.5 transition-all cursor-pointer relative">
      ...
      <div className="flex items-start gap-2 mb-3">
        ...
        {aiGenerated && (
          <span className="text-[11px] bg-[#F5F5FF] text-[#8F8CFF] px-2 py-1 rounded font-medium flex items-center gap-1">
      ...
      <div className="flex items-center justify-between">
        ...
        <button className="text-[12px] text-[#8F8CFF] font-medium opacity-0 group-hover:opacity-100 transition-opacity">
          Open →
        </button>
      </div>
    </div>
...
function PopularCard({ title, views, badge }: PopularCardProps) {
  return (
    <div className="bg-white border border-[#E8E8E6] rounded-lg p-4 hover:border-[#D4D4D2] hover:shadow-sm hover:-translate-y-0.5 transition-all cursor-pointer">
      ...
      <div className="flex items-center justify-between">
        <span className="text-[11px] bg-[#F5F5FF] text-[#8F8CFF] px-2 py-1 rounded font-medium">
          {badge}
        </span>
        <div className="flex items-center gap-1 text-[11px] text-[#71717A]">
          <Eye className="w-3 h-3" strokeWidth={1.5} />
  ...
  return (
    <div className="flex items-center gap-3">
      ...
      <div className="flex flex-col items-center gap-1">
        <ArrowRight className="w-4 h-4 text-[#8F8CFF]" strokeWidth={1.5} />
        <span className="text-[10px] text-[#A1A1AA] font-medium">{type}</span>
      </div>
      <div className="flex-1 bg-[#F5F5FF] rounded-lg p-3 border border-[#E8E7FF]">
