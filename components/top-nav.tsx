...
import { Plus, Video, Upload, Layers, X } from "lucide-react"
    ...
    <>
      <nav className="sticky top-0 z-20 bg-white border-b border-[#E4E4E7]">
        <div className="px-8 py-4 flex items-center justify-end">
          ...
          <button
            ...
            className="flex items-center gap-2 px-4 py-2 bg-[#8F8CFF] hover:bg-[#7B77FF] text-white font-medium text-[14px] rounded-[10px] transition-colors shadow-sm"
          >
      ...
      {showMenu && (
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 backdrop-blur-sm p-8">
          <div className="bg-white rounded-[16px] w-full max-w-[480px] shadow-xl">
    ...
    >
      <div className="flex items-center gap-3">
        <div className={`p-2 rounded-[8px] flex-shrink-0 ${
          primary ? "bg-[#F5F5FF] text-[#8F8CFF]" : "bg-[#F4F4F5] text-[#52525B]"
        }`}>
          {icon}
        </div>
