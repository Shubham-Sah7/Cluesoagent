...
function TrupeerMark({ size = 24 }: { size?: number }) {
  return (
...
export function RecordFlow({ onClose }: { onClose: () => void }) {
                ...
                return (
                  <button
                    ...
                    className={cn(
                      ...
                      active ? "border-[#8F8CFF] bg-[#F5F5FF]" : "border-[#E4E4E7] hover:border-[#D4D4D8] bg-white"
                    )}
                  >
                    {m.recommended && (
                      <span className="absolute top-3 right-3 text-[10px] font-semibold text-[#8F8CFF] bg-[#EEEEFF] px-2 py-0.5 rounded-full">
                        Recommended
                      </span>
                    )}
                    <div className={cn(
                      "w-9 h-9 rounded-xl flex items-center justify-center mb-3 transition-colors",
                      active ? "bg-[#8F8CFF]/15" : "bg-[#F4F4F5]"
                    )}>
                      <Icon className={cn("w-4 h-4", active ? "text-[#8F8CFF]" : "text-[#52525B]")} strokeWidth={1.5} />
                    </div>
                    ...
                    {active && (
                      <span className="absolute top-3 left-3 w-4 h-4 rounded-full bg-[#8F8CFF] flex items-center justify-center">
          ...
          <div className="px-8 pb-8 space-y-5">
                ...
                return (
                  <button
                    ...
                    className={cn(
                      ...
                      active ? "border-[#8F8CFF] bg-[#F5F5FF]" : "border-[#E4E4E7] hover:border-[#D4D4D8]"
                    )}
                  >
                    ...
                    {active && (
                      <span className="w-5 h-5 rounded-full bg-[#8F8CFF] flex items-center justify-center">
            ...
            <div className="flex gap-3 pt-1">
              ...
              <button
                ...
                className="flex-1 flex items-center justify-center gap-2.5 bg-[#8F8CFF] hover:bg-[#7B77FF] text-white font-semibold text-[14px] py-3 rounded-xl transition-colors"
              >
  ...
  if (step === "processing") {
    return (
      <FullPage>
        <div className="w-14 h-14 bg-[#8F8CFF] rounded-2xl flex items-center justify-center mb-10">
          <TrupeerMark size={26} />
        </div>
            ...
            return (
              <div key={label} className={cn("flex items-center gap-3.5 transition-opacity duration-300", done || current ? "opacity-100" : "opacity-25")}>
                <span className={cn(
                  ...
                  done    ? "bg-[#8F8CFF]"               : "border-2 border-[#D4D4D8]"
                )}>
                  {done    && <Check className="w-3.5 h-3.5 text-white" strokeWidth={2.5} />}
                  {current && <span className="w-2 h-2 rounded-full bg-[#8F8CFF] animate-pulse" />}
                </span>
        ...
        <div className="w-full max-w-[360px] h-1 bg-[#F4F4F5] rounded-full overflow-hidden">
          <div
            className="h-full bg-[#8F8CFF] rounded-full transition-all duration-500"
  ...
  if (step === "understanding") {
    return (
      <FullPage>
        <div className="w-20 h-20 rounded-full bg-[#F5F5FF] flex items-center justify-center mb-8">
          <div className="w-12 h-12 rounded-full bg-[#8F8CFF] flex items-center justify-center">
        ...
        <div className="w-full max-w-[380px] space-y-2.5 mb-12">
          {AI_FINDINGS.map((f, i) => (
            <div key={i} className="flex items-center gap-3 bg-[#F9F9FF] border border-[#EEEEFF] rounded-xl px-4 py-3">
              <span className="w-5 h-5 rounded-full bg-[#8F8CFF] flex items-center justify-center flex-shrink-0">
                <Check className="w-3 h-3 text-white" strokeWidth={3} />
              </span>
      ...
      <FullPage className="relative">
        ...
        <button
          ...
          className={cn(
            ...
            selectedOutput
              ? "bg-[#8F8CFF] hover:bg-[#7B77FF] text-white"
      ...
      <FullPage>
        ...
        <div className="w-full max-w-[400px] mb-8">
          <div className="flex items-center justify-between text-[13px] mb-2.5">
            ...
            <span className="text-[#8F8CFF] font-semibold">{Math.round(genProgress)}%</span>
          </div>
          <div className="h-2 bg-[#F4F4F5] rounded-full overflow-hidden">
            <div className="h-full bg-[#8F8CFF] rounded-full transition-all duration-75" style={{ width: `${genProgress}%` }} />
          </div>
        </div>
            ...
            return (
              <div key={label} className={cn("flex items-center gap-3", done || current ? "opacity-100" : "opacity-25")}>
                <span className={cn(
                  ...
                  done    ? "bg-[#8F8CFF]"                : current ? "border-2 border-[#8F8CFF]" : "border-2 border-[#D4D4D8]"
                )}>
  ...
  if (step === "review") {
    ...
    return (
      <FullPage className="relative">
        ...
        <div className="flex items-center gap-2 bg-[#F5F5FF] text-[#8F8CFF] text-[13px] font-semibold px-4 py-2 rounded-full mb-8">
        ...
        <div className="w-full max-w-[680px] aspect-video bg-[#F8F8F7] rounded-2xl border border-[#E4E4E7] mb-10 flex items-center justify-center">
          <div className="text-center">
            <div className="w-16 h-16 rounded-2xl bg-[#8F8CFF]/10 flex items-center justify-center mx-auto mb-4">
              <OutputIcon className="w-8 h-8 text-[#8F8CFF]" strokeWidth={1.5} />
            </div>
        ...
        <div className="flex items-center gap-3">
          ...
          <button className="px-8 py-2.5 text-[14px] font-semibold bg-[#8F8CFF] hover:bg-[#7B77FF] text-white rounded-xl transition-colors">
            Publish
          </button>
          <button className="px-6 py-2.5 text-[14px] font-medium text-[#52525B] border border-[#E4E4E7] hover:bg-[#F5F5F5] rounded-xl transition-colors">
            Share
...
function PrimaryBtn({
  ...
  return (
    <button
      ...
      disabled={disabled}
      className={cn(
        "flex items-center gap-2 bg-[#8F8CFF] hover:bg-[#7B77FF] text-white font-semibold text-[14px] px-6 py-3 rounded-xl transition-colors w-full justify-center",
...
function Toggle({
  ...
  return (
    <div className="flex items-center justify-between px-4 py-3.5">
      ...
      <button
        onClick={onToggle}
        className={cn("relative w-10 h-6 rounded-full transition-colors", enabled ? "bg-[#8F8CFF]" : "bg-[#D4D4D8]")}
      >
        <span
          className={cn(
